...
class ControllerExtensionModuleCristaleShippingNotice extends Controller {
    ...
    public function install() {
        ...
        $this->db->query("INSERT INTO `" . DB_PREFIX . "modification` SET `extension_install_id` = '0', `name` = '" . $this->db->escape($this->language->get('heading_title')) . "', `code` = 'cristale_shipping_notice', `author` = 'Furmedia', `version` = '2.0.4', `link` = 'https://github.com/furmedia/Scheduled-popup-notice-pro-opencart', `xml` = '" . $this->db->escape($this->getModificationXml()) . "', `status` = '1', `date_added` = NOW()");
    }
